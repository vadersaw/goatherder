...
class HomePageTest(TestCase):
    """
    These are the unit tests that go along with the functional tests
    for this particular user story
    """
    
    def test_root_url_resolves_to_home_page_view(self):
        found = resolve('/')
        # found.func is the View that would be used to serve the given URL
        self.assertEqual(found.func, home_page)
        
    def test_home_page_returns_correct_html(self):
        """
        We are trying to validate that the homepage is what we wanted
        it to be from an HTML standpoint.
        """
        request = HttpRequest()
